...
@fig-eta-squared-2 presents partial $\eta^2$ estimates when the effect of $X_1$ is no longer null ($a_1 = 1$). PAR performs poorly for these data, consistently underestimating effect sizes. ART also tends to underestimate effects when $a_2 = 0$; however, as $a_2$ increases, its estimates become more variable and exhibit an overall tendency to increase. RNK and INT yield the most precise estimates. INT is only mildly affected when $a_2 = 8$.
...
fig
```
Scatterplots showing the relationship between effect size estimates (Cohen's $f$) obtained with each method and those from the ground-truth analysis for the main effect of $X_1$, when $a_1$ and $a_2$ vary independently within the range $[-2, 2]$. **Note:** The legend items at the top of the plots are interactive and can be used to filter points and compare the results across methods.
:::
...
fig
```
Scatterplots showing the relationship between effect size estimates (Cohen's $f$) obtained with each method and those from the ground-truth analysis for the main effect of $X_1$, when $a_1$ and $a_2$ vary independently within the range $[-8, 8]$. **Note:** The legend items at the top of the plots are interactive and can be used to filter points and compare the results across methods.
:::
