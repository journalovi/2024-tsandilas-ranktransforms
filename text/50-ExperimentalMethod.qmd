...
<!--
 ...
 While random slope effects can have an impact on real experimental data [@barr:2013], we do not consider them here for two main reasons: (1) to be consistent with previous evaluations of the ART procedure [@elkin:2021]; and (2) because mixed-effects procedures with random slope effects are computationally demanding, adding strain to simulation resources. Consequently, investigating random slopes is beyond the scope of our work, as the simulation procedure is already extensive.

**Encoding the levels of factors.** All factors are treated as categorical, and their levels appear as strings in our simulated data, e.g., "A1", "A2", ... for $X_1$, and "B1", "B2", ... for $X_2$. However, each factor level must be assigned a numerical effect. To this end, we use a sum-to-zero contrast coding scheme, which allows us to calibrate effects across levels. Specifically, to encode the levels of the two factors $x_{1i} \in X_1$ and $x_{2j} \in X_2$ we proceed as follows: 
...
where $\mathcal{F}$ denotes the probability density (or mass) function from which the responses $Y_{ijk}$ are drawn. For example, the mean of the exponential distribution is equal to $1 / \lambda$, where $\lambda$ is its rate parameter. This mean is linked to the linear predictor through the inverse link function $g^{-1}(x) = e^{x}$, which implies $\lambda = e^{-\ell_{ijk}}$. Similarly, for the binomial distribution, the canonical link function is the *logit* function, whose inverse is the *logistic* function $g^{-1}(x) = 1 / (1 + e^{-x})$.
 ...
 @tbl-distributions summarizes how the location parameters of all the distributions are linked to the linear predictor. We emphasize that the Cauchy distribution does not define a generalized linear model (because it does not belong to an exponential family) but we nevertheless use a model that links its location (the median) to the linear predictor via the identity link. 
...
**Magnitude of effects.** We control the magnitude of effects by varying $a_1$, $a_2$, and, for some experiments (those evaluating power for interactions), $a_{12}$. @fig-effects-normal presents normal distributions ($\sigma = 1$) generated across the range of values of $a_1$ used to evaluate Type I error rates. In these examples, we illustrate effects for a factor with three categorical levels and, for clarity, disregard between-subjects variability ($\sigma_s = 0$).
...
In practice, we randomly select the standard deviation $\sigma_s$ --- which determines between-subjects variability --- from the interval $[0.1, 0.5]$, drawing each value from a uniform distribution. Because a different value is sampled at each simulation iteration, our results reflect a distribution of standard deviations, rather than a single fixed value. Note that @elkin:2021 also varied the standard deviation, but only across three fixed values. We believe that sampling from a continuous range is more appropriate.
...
### Evaluation measures
Significance tests involve two types of errors that can occur when we draw conclusions from data. *Type I errors*, or false positives, are mistaken rejections of the null hypothesis. Type II errors, or false negatives, are failures to reject a null hypothesis that is actually false. In our illustrative example in @fig-example, a Type I error occurs when we conclude that the choice of technique affects time performance (i.e., we reject the null hypothesis) when, in fact, it does not. A Type II error, in turn, occurs when we fail to conclude that task difficulty affects time performance (i.e., we do not reject the null hypothesis), even though such an effect is actually present.
