...
plotlyScatter <- function(df, xlab = "p-value (ground truth)", ylab= 'p-value', xvar = "pvalueX1", yvar = paste(xvar, "_", sep ="")) {
  # aesthetics
  ...
  xvarnames <- c(paste(xvar,"3x2",sep="."), paste(xvar,"3x2",sep="."), paste(xvar,"3x2",sep="."))
  yvarnames <- c(paste(yvar,"3x2",sep="."), paste(yvar,"3x2",sep="."), paste(yvar,"3x2",sep="."))
  ...
  createPlot <- function(data, dnames, symbols) {
    ...
    layout(
      ...
      xaxis = list(type = "log", title = xlab, showline=T, mirror = F, range = c(min, 0.1)),
      yaxis = list(type = "log", title = ylab, font = list(size = 13), zeroline = F, showline=T, mirror = F, range = c(min, 0.1))
    )
...
readlyDataPoints <- function(prefix = "6_plot_Effect_Size", distributions=c("norm", "lnorm", "exp", "poisson", "binom", "likert5B"),
  ...
  df <- reshape2::melt(df, id.vars = c("n", "design", "distr", effect), measure.vars = vars, 
      variable.name = "method", value.name = paste(effect, "_", sep=""))
...
prefix <- "7_Likert_scales_2"
df <- readlyDataPoints(prefix,  distributions=c("likert7", "likert7B"), dnames = c("Equal", "Unequal"), effect = "X1")
df_equal <- df[df$distr == "Equal",]
df_equal.art <- df_equal[df_equal$method == "ART",]
df_equal.par <- df_equal[df_equal$method == "PAR",]
df_equal.rnk <- df_equal[df_equal$method == "RNK",]
df_equal.int <- df_equal[df_equal$method == "INT",]

df_unequal <- df[df$distr == "Unequal",]
df_unequal.art <- df_unequal[df_unequal$method == "ART",]
df_unequal.par <- df_unequal[df_unequal$method == "PAR",]
df_unequal.rnk <- df_unequal[df_unequal$method == "RNK",]
df_unequal.int <- df_unequal[df_unequal$method == "INT",]


#fig <- plotlyScatter(df, xvar = "pvalueX1X2")
