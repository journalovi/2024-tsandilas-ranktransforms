---
title: "Introduction - Illustrative example (analysis with LMER)"
...
We use [data](illustrative-example-dataset.csv) from a hypothetical experiment that evaluates the time performance of three user interface techniques (*A*, *B*, and *C*) that help users complete image editing tasks of four different difficulty levels: *Level1*, *Level2*, *Level3*, and *Level4*. 

The experiment follows a fully balanced repeated-measures factorial design, where each participant ($N = 24$) performs 12 tasks in a unique order. The researchers measure the time that it takes participants to complete each task, but participants also evaluate the perceived efficiency, using an ordinal scale with five levels.

**Note:** We describe in detail the simulation process used to generate this dataset [here](illustrative-example-data-generation.html).
...
We also report an analysis with the Friedman test, after aggregating the dataset, either by *Difficulty* or by *Technique*.
```{r, warning=FALSE}
df_diff <- df %>% 
  group_by(Participant, Difficulty) %>% 
  summarise(Perceived_Efficiency = mean(Perceived_Efficiency), .groups = "drop")
friedman.test(Perceived_Efficiency ~ Difficulty | Participant, data = df_diff)

df_tech <- df %>% 
  group_by(Participant, Technique) %>% 
  summarise(Perceived_Efficiency = mean(Perceived_Efficiency), .groups = "drop")
friedman.test(Perceived_Efficiency ~ Technique | Participant, data = df_tech)
``` 

Again, ART is the only method to detect an main effect for both Difficulty and Technique, as well as an interaction effect. Its *p-*values are orders of magnitude lower than the *p-*values returned by the other methods.
