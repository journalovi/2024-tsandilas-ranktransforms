...
We present results from experiments that investigate additional scenarios, as well as other nonparametric methods. Unless explicitely mentioned in each section, we follow the experimental methodology presented in the main article. For most experiments, we focus on $n = 20$. At the end of each section, we summarize our main conclusions.
...
### Conclusion
ART is sensitive to the presence of missing data, when at least $10\%$ of the observations are missing. In particular, we found its Type I error rate for main effects increases further, and this issue also concerns normal distributions. The other methods do not seem to be affected by missing data. However, we emhasize that we only tested missing data that are randomly drawn from the complete set of observations. Results might be different if there was systematic bias in the data imbalance structure.

## Log-normal distributions {#log-normal}
We conduct an experiment to evaluate log-normal distributions with a wider range of $\sigma$ parameters (see @fig-lognormal-distributions), in particular distributions with less variance, which exhibit a lower degree of skew. 
...
**Power: Main effects**. @fig-ATS-power-main-1 and @fig-ATS-power-main-2 present results on the power of the methods to detect main effects. 
...
**Power: Interactions**. @fig-ATS-power-interaction shows results on the power of the methods to detect interactions. INT emerges again as the most powerful method. We observe that the power of ATS is particularly low under the within-subjects design. 
...
Finally, we evaluate the generalizations of nonparametric tests recommended by Lüpsen [-@luepsen:2018; -@luepsen:2023] as implemented in his *np.anova* function [@luepsen_R]. Specificlly, we evaluate the the generalization of the van der Waerden test (VDW), and the generalization of the Kruskal-Wallis and Friedman test (KWF). Their implementation uses R's *aov* function and requires considering random slopes in the error term of the model, that is, using ``Error(s/(x1*x2))`` for the two-factor within-participants design and ``Error(s/x2)`` for the mixed design, where ``s`` is the subject identifier variable. We also used the *aov* function and the same formulation of the error term for all other methods.

**Type I error rates: Main effects**. @fig-vdWaerden-designs-main presents Type I error rates for the main effect of $X_2$. While all methods perform well under the within-subjects and mixed designs, the error rates of VDW and KWF drop drastically as the effect of $X_1$ grows. We will see below that the power of these methods evaporates in these cases.  
...
**Type I error rates: Interactions**. @fig-vdWaerden-designs-interaction-1 presents Type I error rates for the interaction, in the presence of a single main effect. Again, the error rates of VDW and KWF drop quickly for both the between-subjects and mixed design @fig-vdWaerden-designs-interaction-1 presents results when both main effects are present. Under the within-subjects design, KWF is the worst performing techique. VDW performs better than RNK but still lags behind INT. For the other two designs, KFW and VDW yield lower or zero error rates when main effects are large, which can be explained by their extreme loss power in these cases.
