...
plotErrorGrid <- function(df, max = 100, var = "rateX1X2", xvar = "effectX1", xlab, legend = TRUE, ystep = 20) {
	legendPos = ifelse(legend, "top", "none") 
	...
	plot <- ggplot(df, aes(x=factor(eval(as.symbol(xvar))), y=100*eval(as.symbol(var))))+ 
	    ...
	    geom_line(aes(group=method, color=method)) + 
	    geom_point(aes(color=method, shape=method))+
	    scale_y_continuous(limits = c(0, max + 5), breaks = seq(0, 100, ystep), expand = c(0, 0)) +
	    ylab("Type I errors (%)") +
	    xlab(xlab) + 
	    theme_bw() + 
	    theme(axis.text.x = element_text(angle=-60, hjust=0, vjust=0.5)) +
...
plotErrorGridByDesign <- function(df, max = 100, var = "rateX1X2", xvar = "effectX1", xlab, legend = TRUE, ystep = 20) {
	legendPos = ifelse(legend, "top", "none") 

	cbPalette <- c("#888888", "#E69F00", "#009E73", "#FF5E00")
	shapes <- c(1, 2, 3, 8)

	design_levels <- unique(df$design)
	distr_levels <- levels(df$distr)

	labels <- data.frame(
  		distr = rep(distr_levels, length(design_levels)),
  		design = unlist(lapply(1:length(design_levels), function(index){ rep(design_levels[index], length(distr_levels)) })),
  		label = unlist(lapply(1:length(design_levels), function(index){ c(paste("", design_levels[index], sep=""), rep(NA, length(distr_levels) - 1)) }))
	)

	plot <- ggplot(df, aes(x=factor(eval(as.symbol(xvar))), y=100*eval(as.symbol(var))))+ 
		facet_grid(design ~ factor(distr)) +
	    geom_line(aes(group=method, color=method)) + 
	    geom_point(aes(color=method, shape=method))+
	    scale_y_continuous(limits = c(0, max + 5), breaks = seq(0, 100, ystep), expand = c(0, 0)) +
	    ylab("Type I errors (%)") +
