...
**Type I error rates: Interactions**. @fig-vdWaerden-designs-interaction-1 presents Type I error rates for the interaction, in the presence of a single main effect. Again, the error rates of VDW and KWF drop quickly for both the between-subjects and mixed design @fig-vdWaerden-designs-interaction-1 presents results when both main effects are present. Under the within-subjects design, KWF is the worst performing technique. VDW performs better than RNK but still lags behind INT. For the other two designs, KFW and VDW yield lower or zero error rates when main effects are large, which can be explained by their extreme loss power in these cases.
