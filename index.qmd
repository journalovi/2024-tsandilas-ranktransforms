...
### Overview
The above example does not capture a rare phenomenon. We will show that ART's error inflation is systematic for a range of distributions that deviate from normality, both continuous and ordinal. We will explain how the problem emerges. We will also see that ART often performs worse than simpler methods that ART is widely considered to repair or improve, such as the pure rank transformation or no transformation at all. Even worse, while the rank transformation can inflate Type I and Type II errors on interactions, ART struggles to correctly infer both main and interaction effects. 

## Background {#background}

### Nonparametric statistics
Traditional analysis of variance (ANOVA) and t-tests are commonly known as *parametric* statistical procedures because they make assumptions about the underlying data distributions and their parameters, such their means and standard deviations. In contrast, *nonparametric* statistical procedures make fewer or minimal assumptions about the underlying distributions. Representative examples of nonparametric methods are the Mann-Whitney U test [@Mann–Whitney-U] and Kruskal–Wallis test [@Kruskal-Wallis] for independent samples, and the Wilcoxon sign-rank test [@Wilcoxon] and Friedman test [@Friedman] for paired samples or repeated measures. These methods are widely used in research as they allow to work with distributions of arbitrary shapes, while real data often deviate from the gaussian model (normal distribution) or violate other assumptions, such as the homogeneity of variance or the sphericity assumption. 

### Rank transformations and nonparametric tests for designs with multiple factors
Despite their merits, common nonparametric methods pose several constraints, such as they support simple experimental designs with one only factor. This is a major limitation compared to ANOVAs or the more versatile mixed-effect models [@Kaptein:2016]. 

Rank transformations [@conover:1981] aim to close the gap between nonparametric statistics and ANOVAs, and other linear models. The simple rank transform consists in sorting the raw observations and replacing them by their ranks. In case of ties, tied observations are assigned their average rank. This simple process creates a uniform distribution, which allows to directly apply an ANOVA. The reason is that the normality assumption of ANOVA concerns the sampling distribution of the mean, not the distribution of individual population values. And fortunately, for uniform distributions, the sampling distribution of the mean is close to normal even very small samples.

@conover:1981 has shown that ... 
...
Alternative nonparametric approaches such as ATS [@Brunner_ATS:2021], which was proposed for Likert scales in HCI by @Kaptein:2010. 
