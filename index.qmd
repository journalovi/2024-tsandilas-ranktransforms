...
We do not directly evaluate Type II errors. Instead, we report on statistical *power* defined as $Power = 1 - \beta$, where $\beta$ is the rate of Type II errors. Significance tests do not provide any power guarantees. However, we can compare the power of different methods to evaluate their relative performance. As there is a tradeoff between power and Type I errors, we assess power with respect to Type I error rates. If Type I error rates are inflated beyond acceptable levels, power comparions are misleading, so we avoid them.

In addition to power, we assess effect size estimates, focusing on the partial $\eta^2$ measure, which is commonly used in ANOVA. To evaluate the quality of estimates of different methods, we measure how well they correlate with the estimates of the optimal ground-truth method, such as a parametric ANOVA under a normal distribution. 

We present our results on power and effect sizes in [Section 8](#power). 

### Iterations and apparatus
Our power and Type I error rate estimates are based on $5000$ iterations per population configuration and per sample size. Our experimental code is written in R and is available in our supplementary material. 
Our different experiments were run seperately in a cluster of 8 machines Dell R640 Intel Xeon Silver 4112 2.6GHz with 4 cores and 64 GB memory. Our R code was parallelized to use all four cores of each machine. Some experiments took a few hours to complete, while others took several days. 
...
## Ratio scales {#ratio}
Our first series of experiments evaluate the Type I error rate of the methods for ratio variables. We test four continuous and two discrete distributions with a fixed set of parameters.  

1. The normal distribution $N(\mu, \sigma)$ with parameters $\mu = 0$ and $\sigma = 1$. 

2. The log-normal distribution $LogN(\mu, \sigma)$ with parameters $\mu = 0$ and $\sigma = 1$. It is a highly skewed distribution. As discussed in [the introduction](#intro), the log-normal distribution is a good model for a range of measures that are bounded by zero, such as task-completion times. 

3. The exponential distribution $Exp(\lambda)$ with a parameter $\lambda = 2$. 

4. The Cauchy distribution $Cauchy(x_0,\gamma)$ with parameters $x_0 = 0$ and $\gamma = 1$ 

5. The Poisson distribution $Pois(\lambda)$ with a parameter $\lambda = 3$

6. The binomial distribution $B(n,p)$ with parameters $n = 10$ and $p=.1$ 
 
For this series of experiments, we study a 4 $\times$ 3 repeated-measures design. In the following, we refer to the first factor with the four levels as $X_1$ and the second factor with three levels as $X_2$.
