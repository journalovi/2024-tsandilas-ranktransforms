...
This is not the only alignment technique found in the research literature. @Sawilowsky:1990 discusses several variations of aligned rank-based tests. @Sawilowsky:1990 also suggests that at least for balanced designs, interactions could be removed when aligning main effects, in the same way main effects are removed when aligning interactions. This approach is also taken by @Leys:2010, who derive a common ranking for both main effects after substracting the interation term. We do not evaluate these alternative alignment methods in this paper.

**The inverse normal transform.** 
A third tranformation method that we evaluate is the rank-based inverse normal transformation (INT). More than 70 years old [@VanDerWaerden:1952], it has appeared in several variations [@Beasley:2009,@Solomon2009]. However, its general formulation is as follows: 

$$ 
INT(Y) = \Phi^{-1}(\frac{rank(Y) - c}{N - 2c + 1})
$${#eq-int}

where $N$ is the total number of observations and $\Phi^{-1}$ is the standard normal quantile function, which aims to transform the uniform distribution of ranks into a normal distribution. Different authors have used a different parameter $c$. In our experiments, we use the *Rankit* formulation [@Bliss:1956], where $c = 0.5$, because past simulation studies [@Solomon2009] have shown that it is more accurate than alternative formulations. However, @Beasley:2009 report that the choice of $c$ has minor importance. We implement the INT method in R as follows: 
...
**Other rank-based tests.** Alternative nonparametric approaches such as ATS [@Brunner_ATS:2021], which was proposed for Likert scales in HCI by @Kaptein:2010. 
...
### Evaluations of ART and past warnings
