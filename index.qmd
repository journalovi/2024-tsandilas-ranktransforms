...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "range of effects (g)", var = "cohensfX1", xvar = "effectX1", min = -6, max = 103, ytitle = 'R squared')
```
...
We also conducted an analysis with a cumulative probit model, using a Bayesian statistics framework [@Burkner:2019], without informative priors. For an extensive discussion about why using a Bayesian framework for this type of models, we refer interested readers to @Liddell:2018. We describe the model in R as follows:
...
@Liddell:2018 recommend using ordered-probit models for the analysis of Likert scales, and @Burkner:2019 further demonstrate how to fit such models. Here, we apply their approach to analyze participants' responses for intrinsic motivation [@Karolus:2023]. To this end, we do not average the individual Likert items of the scale. We treat instead items and participants as a random effects through the following mixed-effects model:  
...
However, @Carifio:2008 also stated that analyzing individual Likert items *"is a practice that should only occur very rarely."* This statement was criticized by @Norman:2010, who argued that it does not convincingly refute the *"ordinalist"* position. According to the author, since ANOVA and other parametric statistics are robust enough to deal with Likert data, even with small sizes and unequal variances, there is *"no fear of coming to the wrong conclusion."* In the same line, @Harper:2015 came up with a number of recommendations about these issues, including the following two: (1) *"Individual rating items with numerical response formats at least five categories in length may generally be treated as continuous data"*; and (2) *"Consider nonparametric or categorical data analysis approaches for individual rating items with numerical response formats containing four or fewer categories"*. However, @Liddell:2018 identified several situations in which parametric tests may fail, even when applied to averages of multiple ordinal items. For example, the authors demonstrated how they can inflate errors when variances are not equal or deform interaction effects. 

Our experimental results on individual Likert items with 5, 7, and 20 levels show that parametric ANOVA is surpisingly robust for the analysis of main effects, as long as assumptions of equal variances hold. On the other hand, we observed that parametric ANOVA method often fails to correctly infer interactions when main effects become large, and the problem becomes more apparent when ordinal thresholds are not equidistant. Furthermore, the method inflates Type I error rates when applied to factors with unequal variances. INT appears to reduce the impact of these problems but without offering a satisfying solution. We also found that at least for 5-level items with unequal threshold distances, all methods may lead to unprecise effect size estimates. In conclusion, while ANOVA seems to be a valid method for analyzing responses to individual Likert items, there are situations that require special attention. It is also important to emphasize that a nonparametric approach may not necessariy work well in these situations. 

We demonstrated how to use probit 
