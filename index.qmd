...
>>>>>>> 1e19a88fc40600a4fe78c5f93c268a1fad3da989

In addition to the main sections of the paper, we provide two appendices: Appendix I presents results from additional Monte Carlo experiments, and Appendix II details analysis alternatives for representative dataset examples.

## Background {#background}
Traditional analysis of variance (ANOVA), t-tests, and linear regression models are commonly known as *parametric* statistical procedures because they make assumptions about the underlying data distributions and their parameters, such as their means and standard deviations. A common assumption is that the *sampling distribution* of the mean follows a normal distribution. If sample sizes are sufficiently large, this assumption is often reasonable because the central limit theorem predicts that the distribution of the sample mean is assympotically normal, irrespective of the shape of the population distribution. However, in small samples, the accucary of these methods can drop substantially when populations deviate from normal. The goal of *nonparametric* statistical procedures is to offer more generic analysis methods that make fewer or no assumptions about the underlying distributions and thus do not suffer from these problems.

### Nonparametric statistical tests
The original idea of replacing the data values by their ranks belongs to @spearman1904. The key advantage of ranks is that they preserve the monotonic relationship of values, while they also permit to derive an exact probability distribution. This propability distribution can then be used for statistical inference, such as to calculate a *p*-value. 

The idea of using ranks was borrowed by other authors, leading to a range of nonparametric tests that are commonly used today. Representative examples include the Mann-Whitney U test [@Mann-Whitney-U] and the Kruskal–Wallis test [@Kruskal-Wallis] for independent samples, and the Wilcoxon sign-rank test [@Wilcoxon] and the Friedman test [@Friedman] for paired samples and repeated measures. 

### Rank transformations
The above nonparametric methods pose significant analysis constraints, as they only support simple experimental designs, e.g., containing a single independent variable (factor). Rank transformations aim to close the gap between nonparametric statistics and ANOVA.

**The rank transform.** The simple rank transform consists of sorting the raw observations and replacing them by their ranks. In case of ties, tied observations are assigned **fractional ranks** equal to their average order position in the ascending order of values. For example, the two $0.3$ instances of the $Y$ responses in @fig-ART-explained (a), which are the lowest values in the dataset, receive a rank of $1.5$ (see $rank(Y)$), while the next value (a $0.4$) in the sequence receives a rank of $3$.
...
Finally, we chose to limit our investigation to balanced experimental designs. The rationale behind this decision is that we have not encountered any prior claims about ART's suitability for unbalanced data, and the ARTool [@artool] issues a warning in such situations. However, our appendix presents additional results on experimental designs with missing data, which confirm that in such situations, ART's accuracy may deteriorate further.
...
Our tests in Appendix I also show that ART's Type I error rates increase further in unbalanced designs with missing data.
...
Nevertheless, we found that even for one-factor designs, INT turns to be more powerful than these nonparametric tests. 

Consequently, our recommendation aligns with Gelman's advice [@gelman2015int_vs_wilcoxon] to use INT as a general nonparametric method, with careful consideration of potential cases of failure and a cautious interpretation of interaction effects. 

### Priviledging parametric methods
The scope of rank-based nonparametric statistics is very limited. @baguley2012 summarizes a fondumental concern regarding their use as follows: 
...
For example, one can log-transform data with positively skewed distributions, or use generalized mixed-effects models, which provide support for a broader set of data distributions. 
