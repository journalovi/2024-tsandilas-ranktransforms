...
Then, we compute the correlation ($cor$) between the method estimates and the ground truth over 1000 iterations. In each iteration, we randomly draw the magnitude of all three effects ($a_1$, $a_2$, and $a_{12}$) from a range $[-g, g]$, where $g$ varies in $\{0, 0.5, 1, 2, 4, 8\}$. These correlation values serve as measures of the quality of effect size estimates. We present the results for a sample size of $n=20$.

**Main effects**. @fig-effect-main-1 depicts correlation trends for the first factor $X_1$. All methods demonstrate high correlation levels under the normal distribution. INT performs exceptionally well across all continuous distributions but experiences a performance decline in overpowered studies, when the magnitude of effects is high. PAR is ranked as the least effective method under the log-normal and exponential distributions, followed by ART. ART's performance is particularly low under the binomial and ordinal distribution. 
...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "range of effects (g)", var = "etaX1", xvar = "effectX1", min = 34, max = 103, ytitle = 'cor (%)')
```
Correlation (%) between effect size estimate (partial $\eta^2$) of each method and estimate of the ground-truth method for main effect on $X_1$, as a function of $g$.
:::
...
fig
```
Scatterplots showing the relationship between effect size estimates (partial $\eta^2$) of each method and estimates of the ground-truth method for main effect on $X_1$ ($g = 2$). We show results for the normal, log-normal, and ordinal data.  
:::
...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "range of effects (g)", var = "etaX2", xvar = "effectX2", min = 34, max = 103, ytitle = 'cor (%)')
```
Correlation (%) between effect size estimates (partial $\eta^2$) of each method and estimates of the ground-truth method for main effect on $X_2$
:::
...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "range of effects (g)", var = "etaX1X2", xvar = "effectX1X2", min = 25, max = 103, ytitle = 'cor (%)')
```
Correlation (%) between effect size estimates (partial $\eta^2$) of each method and estimates of the ground-truth method for interaction effect, as a function of $g$.
:::
...
It is important to note, however, that the aforementioned *p*-values are outcomes of Type III hypothesis tests. While these tests are deemed more suitable for unbalanced data when interaction effects emerge, their use remains a subject of debate [@Smith:2022], and some experts argue that interpreting main effects in the presence of interactions may not be meaningful [@Hector:2010]. Additionally, the ARTool issues a warning for the above analysis:

> *F values of ANOVAs on aligned responses not of interest are not all ~0. ART may not be appropriate.*

This warning could be a sufficient reason to refrain from using the method with these data.
...
We observe that ART exhibits significant challenges with such binary data, as its error rates escalate to very high levels for all main and interaction effects. In contrast, PAR consistently maintains error rates close to nominal levels. Despite the higher rate of Type I errors, ART seems to detect main effects on $X_2$ and $X_3$ with lower power compared to PAR. For example, for $a_2 = 0.5$ (main effect of $X_2$), its power is $79\%$ compared to a power of $90\%$ for PAR ($\alpha = .05$). 
