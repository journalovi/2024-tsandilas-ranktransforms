...
It is important to note, however, that the aforementioned *p*-values are outcomes of Type III hypothesis tests. While these tests are deemed more suitable for unbalanced data when interaction effects emerge, their use remains a subject of debate [@Smith:2022]. Some experts argue that interpreting main effects in the presence of interactions may not be meaningful [@Hector:2010]. Additionally, the ARTool issues a warning for the above analysis:
...
**Simulation with balanced data.** What if the exerimental design was balanced and this warning did not appear? What are the risks of using ART with such binary responses? We run a Monte Carlo experiment to understand the problem. 

As @Martin:2023, we investigate a $4 \times 2 \times 2$ repeated-measures design and set the number of participants to $n=22$. We transform the latent variable to a Bernoulli distribution with a success probability parameter $p=.46$ and then assess the Type I error rate of ART in the absence of any effect. We also assess the the Type I error rate of regular ANOVA with no transformation (PAR). When responses are binary, RNK and INT do not alter the data, leading to results that coincide with those of PAR.
...
We observe that ART exhibits significant challenges with such binary data, as its error rates escalate to very high levels for all main and interaction effects. In contrast, PAR consistently maintains error rates close to nominal levels. Despite the higher number of Type I errors, ART seems to detect main effects on $X_2$ or $X_3$ with lower power compared to PAR. For example, for an effect of $a_2 = 0.5$, its power is $79\%$ compared to a power of $90\%$ for PAR ($\alpha = .05$). 
