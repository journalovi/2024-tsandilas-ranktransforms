...
###### Results
Our results indicate that ART systematically confounds effects when the assumptions of normality and equal variances are violated. In such cases, an effect on one factor may cause ART to incorrectly detect effects on other factors or their interactions. The method leads to additional false positives in discrete distributions, particularly when the number of discrete values is small. Moreover, it fails to address scaling issues when testing interactions, resulting in a rapid escalation of Type I error rates when data distributions are ordinal or skewed. [there is maybe some abuse of language in this paragraph (and below) between the type of data (continuous vs discrete (including ordinal)) and their distribution (normal, skewed...)]{.g}
