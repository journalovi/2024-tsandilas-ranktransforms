...
We follow the approach of @DeBruine:2021 and use the R package *faux* v1.2.1 [@faux] to simulate data for our mixed-effects models. We assume that the distributions of random intercepts and errors are normal, or $s_k \sim N(0,\sigma_s)$ and $\epsilon_{ijk} \sim N(0,\sigma)$. To simulate the observed variable $Y'$, we transform the response values $y_{ijk}$ through the approach we presented in [Section 3](#approach). This allows us to generate observed responses for a range of distributions, while we always control for effects in the exact same way.
...
As we explained earlier, interaction effects are deformed when we transform the latent variable $Y$ to obtain the observed responses. Such transformations also affect the Type I error rates and power that we observe. Here, we are interested in evaluating these measures with respect to the effects we apply to the latent variable. In our analysis, we make this distinction whenever it is relevant.  
