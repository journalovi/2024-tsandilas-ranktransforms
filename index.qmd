...
Our fifth experiment compares the statistical power of the four methods on the three 2-factor designs. Because there is a tradeoff between Type I and Type I errors, high power can simply be the result of a high Type I error rate. Since parallel effects can inflate errors (see our previous results), we focus here on single effects, either main or interation effects. We only report results for $n=20$ (see supplementary materials for additional results). 
...
As @Martin:2023, we investigate a $4 \times 2 \times 2$ repeated-measures design and set the number of participants to $n=22$. However, we now test a perfectly balanced design. We transform the latent variable to a Bernoulli distribution with a success probability parameter $p=.46$ (average success rate found by @Martin:2023) and then assess the Type I error rate of ART in the absence of any effect. We also assess the the Type I error rate of regular ANOVA with no transformation (PAR). When responses are binary, RNK and INT do not alter the data, leading to results that coincide with those of PAR. Our results over 5000 iterations are as follows:
