...
### Abandoning ART
Our experimental results corroborate Lüpsen's earlier warnings [@luepsen:2017;@luepsen:2018]. They overwhelmignly demonstrate that the ART method is defective. Its advantages over the simple rank transformation (RNK) are marginal and only applicable to normal distributions with equal variances. However, ART introduces additional risks for a broad spectrum of non-normal distributions. Specifically, it confounds main effects and inflates the Type I error rate of both main effects and interactions. Furthermore, the risks associated with ART are more severe compared to those associated with parametric ANOVA. While skewed distributions lead to a loss of statistical power with parametric ANOVA, ART consistently inflates Type I error rates. In light of these findings, we conclude that ART is an outdated procedure and strongly advise researchers to discontinue its use.
 
Some researchers may be tempted to seek alternative formulations of ART. For example, @Salter:1993 find that using ART with median alignment (i.e., using medians instead of means to align values) corrects the method's instability under the Cauchy distribution. 

### Looking for alternative methods
Given the widespread adoption of ART, many researchers may feel purplexed about which non-parametric alternative method to use. Some may see the inverve normal transformation (INT) as the best candidate for replacing ART. Our results show it to behave well in most configurations that we studied and be the winner among rank tranformations. However, like other rank-based methods, INT still struggles to correctly infer interactions in the presence of parallel main effects, while the method also tends to underestimate large effect sizes. We found that these two problems become more apparent when distributions are discrete. @Beasley:2009 present additional cases 


also observe that INT may inflate Type I errors when designs are imbalanced.  
