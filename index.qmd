...
@fig-effect-main-scatterplots-2 shows the same relationship but for a larger range of effects ($g=8$). Notice that effects sizes are severely distorted by all four methods. We invite the reader to zoom in on the lower range of values ($\eta^2 < 0.6$), where ART's estimates for the two non-normal distributions appear quasi-random.
