...
@fig-power-main-2 presents results for $X_2$. Given the different number of levels for this factor, we now assess a different range of power for each design. Nevertheless, we obseve simmilar patterns. INT emerges as the most powerful method. Although ART surpasses PAR under the log-normal and exponential distributions, it does not demonstrate any other notable advantages compared to the other methods.  
...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "max magnitude of effects", var = "etaX1", xvar = "effectX1", min = 34, max = 103, ytitle = 'cor (%)')
```
Correlation (%) between effect size estimate (partial $\eta^2$) of each method and estimate of the ground-truth method for main effect on $X_1$, as a function of $\gamma$.
:::
...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "max magnitude of effects", var = "etaX2", xvar = "effectX2", min = 34, max = 103, ytitle = 'cor (%)')
```
Correlation (%) between effect size estimate (partial $\eta^2$) of each method and estimate of the ground-truth method for main effect on $X_2$, as a function of $\gamma$.
:::
...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "max magnitude of effects", var = "etaX1X2", xvar = "effectX1X2", min = 25, max = 103, ytitle = 'cor (%)')
```
Correlation (%) between effect size estimate (partial $\eta^2$) of each method and estimate of the ground-truth method for interaction effect, as a function of $\gamma$.
:::
