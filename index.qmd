...
We compare four different versions of linear mixed-effects models (or ANOVA): (i) without transformation (*PAR*); (ii) with a rank transformation (*RNK*); (iii) with an inverse normal transformation (*INT*); and (iv) *ART*. We conduct a series of Monte Carlo experiments that assess their performance under a variety of experimental configurations:
