...
In their ART model, the authors treated the participant identifier and the task identifier as random effects. We could re-run their analysis and replicate their results from Table 1 [@Martin:2023]. However, we also run the same analysis with two additional methods: (i) fitting a linear mixed-effects model (LMER function) without any transformation; and (ii) fitting a binomial generalized mixed-effects model (GLMER function with binomial distribution family). The latter method basically performs a logistic regression and is the most appropriate parametric procedure for this type of data. Below, we present the *p*-values returned from each method:
...
As @Martin:2023, we report results of Type III hypothesis tests for all methods. They are considered as more appropriate when experimental designs are unbalanced, while there are strong interactions between factors. 
