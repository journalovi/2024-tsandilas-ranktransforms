...
```
$R^2$ scores assessing the relationship between estimates (partial $\eta^2$) of each method and estimates of the ground truth method for the main effect on $X_1$. We express these scores as a function of the range of the magnitude of effects $g$.
:::
...
```
$R^2$ scores assessing the relationship between estimates (partial $\eta^2$) of each method and estimates of the ground truth method for the main effect on $X_2$. We express these scores as a function of the range of the magnitude of effects $g$.
:::
...
```
$R^2$ scores assessing the relationship between estimates (partial $\eta^2$) of each method and estimates of the ground truth method for the interaction effect $X_1 \times X_2$. We express these scores as a function of the range of the magnitude of effects $g$.
:::


## Case studies {#casestudies}

### Binary responses: Object-manipulation detection [@Martin:2023]
...
: *p*-values keeping two only significant digits - Type III hypothesis tests {.sm}

There are notable differences among the results obtained from the three methods. Specifically, GLMER provides evidence for a main effect of Type ($p=.036$ compared to $p=.92$ for ART) and an interaction effect Type $\times$ Distance ($p=.013$ compared to $p=.71$ for ART). In contrast, its $p$-value for the main effect of distance is significantly larger ($p=.29$ compared to $p=.031$ for ART). These discrepancies may raise questions regarding the key conclusions of the paper regarding the influence of the above factors.
...
We have not examined how design imbalance affects the accuracy of ART and the other rank-based methods. 

**Simulation with balanced data.** What if the experimental design were balanced, and this warning did not appear? What risks would arise from using ART with such binary responses? To delve into this issue, we conduct a new Monte Carlo experiment.
...
### Likert items: Sense of ownership [@Rosso:2023]
...
effect between Coupling and Perspective (Df residual = 147, F = 5.232, p < 0.05) revealed that the increase in ownership relative to the 2nd person perspective was significantly stronger when participants were coupled.* [@Rosso:2023]
...
: *p*-values keeping two only significant digits {.sm}
...
```
 ...
 For an extensive discussion about why using a Bayesian framework for this type of models, we refer interested readers to @Liddell:2018. This method has not been common in the HCI literature, but we have previously used for the analysis of ordinal responses in two user studies [@fages:2022].
...
One may notice that a single entry (out of 152 entries) in the above dataset is missing, so the design is slightly unbalanced and the ARTool again outputs a warning. Could this imbalance explain ART's failure? The answer is negative. If we complete the missing row with a neutral value (e.g., a 3 in the 5-level ordinal scale) to dismiss the warning, the resulting *p*-values will still be very similar to the ones reported above. 
