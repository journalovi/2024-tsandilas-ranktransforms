...
Like @Martin:2023, we investigate a $4 \times 2 \times 2$ repeated-measures design and set the number of participants to $n=22$. However, we now test a perfectly balanced design. We transform the latent variable to a Bernoulli distribution with a success probability parameter $p=.46$ ---average success rate found by @Martin:2023--- and then assess the Type I error rate of ART in the absence of any effect. We also assess the Type I error rate of regular ANOVA with no transformation (PAR). When responses are binary, RNK and INT do not alter the data, leading to results that coincide with those of PAR. Our results over 5000 iterations are as follows:

|  | $X_1$ | $X_2$ | $X_3$ | $X_1 \times X_2$ | $X_1 \times X_3$ | $X_2 \times X_3$ | $X_1 \times X_2 \times X_3$ |
|:------:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|
| PAR | $4.7$ | $5.1$ | $4.8$ | $5.7$ | $4.8$ | $4.9$ | $4.4$ |
| ART | $21.2$ | $20.8$ | $20.0$ | $22.0$ | $21.5$ | $19.7$ | $21.3$ |
: Type I error rates (%) for $\alpha = .05$ (nominal error rate $=5\%$) {.sm}

|  | $X_1$ | $X_2$ | $X_3$ | $X_1 \times X_2$ | $X_1 \times X_3$ | $X_2 \times X_3$ | $X_1 \times X_2 \times X_3$ |
|:------:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|
| PAR | $1.0$ | $1.1$ | $1.1$ | $1.2$ | $1.1$ | $1.0$ | $0.9$ |
| ART | $9.5$ | $9.4$ | $9.5$ | $10.5$ | $9.7$ | $9.5$ | $9.2$ |
: Type I error rates (%) for $\alpha = .01$ (nominal error rate $=1\%$) {.sm}
...
Even when parametric ANOVA is clearly unsuitable, there are often alternative methods for addressing violations of statistical assumptions. For example, one can log-transform data with positively skewed distributions, or use generalized mixed-effects models, which provide support for a broader set of distributions. Finally, employing simulations, as we did in this article, is an excellent method for evaluating the impact and risks of assumption violations and determine which data analysis procedures to use. 
...
### Interpreting interactions
 We discussed the problem of removable interactions [@Loftus:1978] and demonstrated that even nonparametric methods struggle to control Type I errors of interactions in the presence of parallel main effects. Thus, a positive test for interaction does not necessarily indicate a genuine interaction when tests on main effects also yield positive results. It is crucial to emphasize that if distributional assumptions are violated, larger sample sizes will not mitigate this issue; instead, the ratio of false positives will increase with sample size, as the test will become more sensitive in detecting removable interactions. 
  
A good way to diagnose such problems is to plot interactions (e.g., see @fig-interactions) and visually inspect the trends of the effects. @Loftus:1978 observes that interactions that do not cross over are removable under monotonic transformations, and therefore they cannot be interpreted. In constrast, interactions that cross over are nonremovable. @fig-removable-interactions presents examples of a removable and a nonremovable interaction. Notice that the crossover of the nonremovable interaction is only visible on the second chart (orange), where each line shows the effect of Technique.
...
fig
```
Examples of a removable (left) and a nonremovable interaction (right). A removable interactions does not cross over regardless of how it is plotted. 
:::

While these visual methods help interpret interations, we encourage researchers to use transformation procedures and latent variable models that describe their data generation processes more accurately than one-size-fits-all (either parametric or nonparametric) methods. For instance, time responses following log-normal distributions are best studied after log-transformation. Likewise, binary or ordinal data are best analyzed through logistic regression or ordered probit models. These methods link the observed responses with a normal latent variable, where interactions can then be studied on a common scale. 

### Keeping it simple
Furthermore, planning the analysis methods before data collection is a sensible practice, as it helps focus on measures that are easy to analyze with existing tools and anticipate the types of their generated responses.
