...
Overall, ART operates as expected only under normal distributions with equal variances. In all other scenarios, it performs less effectively than the simpler rank transformations we evaluated, while in many instances, such as with ordinal data, it performs notably worse than parametric ANOVA. Our analysis of real-world data further demonstrates that the use of ART often leads to incorrect conclusions. Conversely, the inverse normal transformation emerges as the most versatile and robust nonparametric method. However, we also identify situations in which this method can also fail.

###### Conclusion
We conclude that ART is a problematic statistical method and should be abandoned. We recommend using parametric methods, such as linear models and their generalizations, whenever it is possible. We propose the inverse normal transformation as a generic nonparametric alternative but caution researchers about its limitations.
...
Early Monte Carlo experiments in the 1990s [@Salter:1993;@Mansouri:1995] and more recent studies [@elkin:2021]have suggested that ART is a robust alternative to ANOVA when normality assumptions are violated. These results have contributed to ART's reputation as a well-established method. However, other research [@luepsen:2017; @luepsen:2018] has raised concerns about the robustness of the method, demonstrating that ART fails to control the Type I error rate in many scenarios. These warnings have not received sufficient attention.

Given the widespread use of ART in HCI research, understanding its associated risks is imperative. This paper presents the outcomes of a series of Monte Carlo experiments. We adopt a distinctive simulation methodology grounded in latent variable modeling. This approach enables us to simulate effects consistently across a broad spectrum of distributions, both discrete and continuous. Furthermore, it allows us to address scaling issues in interpreting interactions. To ensure the clarity of our findings and facilitate their reproducibility, we disaggregate the experimental process into multiple smaller experiments. Each experiment focuses on a distinct variable (e.g., sample size, experimental design, variance ratio) or measure (Type I error rate, power, precision of effect size estimates).
...
Nevertheless, the increased difficulty of the task does not simply translate the distribution to the right. The shape and scale of the distribution also change --- the variance increases, and the mean and median do not increase by the same amount. This poses a probem for ART's alignment procedure, which relies entirely on means. In particular, the more extreme values of a random sample that appear near the further right of the wider distribution (depicted in green in @fig-distributions) will have a significant impact on the calculation of the within-cell mean. This will lead to the exaggeration of all ranks within this cell. 

Unfortunately, an additional issue arises that we need to discuss in more depth. Such non-linear transformations complexify the interpretation of interactions.
...
There are notable differences among the results obtained from the two methods. It is important to note that the aforementioned *p*-values are outcomes of Type III hypothesis tests. While these tests are deemed more suitable for unbalanced data when interaction effects emerge, their use remains a subject of debate [@Smith:2022], and some experts argue that interpreting main effects in the presence of interactions may not be meaningful [@Hector:2010]. Thus, the presence of interactions in the above unbalanced design complexifies the detection and intepretation of main effects. We emphasize that the ARTool raises a warning for such unbalanced designs:
...
A more adapted approach for this type of data would be to use a generalized linear model (GLMER) with a *binomial link function*, which essentially performs a logistic regression. The key adavantage of such models is that they are in line with the latent variable approach, where the link function establishes a connection between a normally distributed latent psychological variable (e.g., the human ability to detect changes in a virtual environment) and the observed variable (e.g., successes or failures in sequential change detection tests). Consequently, we expect them to exhibit stronger predictive power than regular linear models and support a more appropriate interpretation of interaction effects. However, the application of such models requires additional steps, such as comparing alternative models using different criteria [@Bolker2009glm] that are often difficult to evaluate. We thus do not incude them in our simulations here.
...
We can build a simpler regression model by setting ```threshold = "equidistant"``` and compare the two models, e.g., using the Akaike information criterion (AIC). For this specific example, both models lead to almost identical parameter estimates.
