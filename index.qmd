...
**The rank transform.** The simple rank transform consists in sorting the raw observations and replacing them by their ranks. In case of ties, tied observations are assigned their average rank. @conover:1981 showed that for one-factor designs, using ANOVAs on these ranks produces tests that are equivalent or good replacements of traditional nonparametric tests (see [Section 3](#interpretation)). However, a series of studies in the 80s raised caution flags on the use of the method, showing that it may confound main and interaction effects in two-factor and three-fractor experimental designs. For an extensive review of these studies, we refer readers to @Sawilowsky:1990.

**Aligned rank transforms.** Given these negative results, many researchers turned to aligned (or adjusted) rank transformations. @Sawilowsky:1990 discusses a range of aligned rank-based transformation tests, where most focused on interactions. The intuition behind the the align rank transform is that the influence of main effects can removed from the interaction by subtracting their observed means. @higgins:1990 detail the method for two-way designs, while @wobbrock:2011 explain its generalization to more than two factors. We briefly present the key steps of the alignment technique, as it is necessary for understanding its main shortcomings.  
...
To simulate populations in which interactions emerge in the absence of main effects, we examine perfectly symmetric cross-interactions. To this end, we slighly change the method we use to encode the levels of each factor, such that levels are uniformly positioned around 0. For example, for a factor with three levels, we numerically encode the levels as $\{-0.5, 0, 0.5\}$, For a factor with four levels, we encode them as $\{-0.5, -0.1667, 0.1667, 0.5\}$. 

**Interaction effect only**. We first test how the interaction effect alone influences the Type I error rate on $X_2$. @fig-interactions-main-2 presents our results. We observe that both PAR and ART fail for many configurations. Error rates are especially high in the case of the $4 \times 3$ within-subjects design and the $2\times 4$ mixed design under the log-normal and exponential distribution. ART exhibits the worst performance. In constrast, RNK and INT keep error rates close to nominal levels. When interaction effects become large ($a_{12} > 4$), they only start inflating errors under the binomial and ordinal scale. 
