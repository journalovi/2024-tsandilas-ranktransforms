...
However, our experimental findigns and examples reveal that ART is defective, as it performs adequately only with normal distributions and under the assumption of equal variances. These results corroborate previous warnings regarding ART's performance [@luepsen:2017;@luepsen:2018], leading us to conclude that the method should be abandoned. We recommend prioritizing parametric methods, which, thanks to generalized linear and latent variable models, offer versatility beyond traditional ANOVA models. While we propose the inverse normal transformation (INT) as a generic nonparametric alternative, we caution researchers about the limitations of rank-based transformation methods.
