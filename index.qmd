...
In our final experiment, we assess how well each method estimates standardized effect sizes, where we use as ground truth the estimates of a parametric ANOVA conducted on the latent variable $Y$.
While partial $\eta^2$ is the most commonly used effect size measure, we chose to base our analysis on Cohen's $f$ because it is expected to be proportional to the real magnitude of effect. Cohen's $f$ can be derived directly from $\eta^2$ as follows:

$$ 
f = \sqrt{\frac{\eta^2}{1 - \eta^2}}
$${#eq-cohensf}

**Main effects**. We first evaluate estimates of the main effect of $X_1$ as the magnitude of both main effects varies. As a first step, we focus on the normal, log-normal, and ordinal distributions. @fig-effect-main-scatterplots-1 illustrates the relationship between the Cohen's $f$ obtained with each method and the $f$ of the ground truth method, when $a_1$ and $a_2$ are within the range $[-2, 2]$. The proximity of data points to the black diagonal indicates the closeness of estimates to the ground truth. Points above the line are likely to overestimate the effect size, while points under the line are likely to underestimate it. 
...
fig
```
Scatterplots showing the relationship between effect size estimates (Cohen's $f$) of each method and estimates of the ground-truth method for the main effect of $X_1$, when the magnitude of the three effects is within the range $[-2, 2]$. 
:::

We observe that PAR tends to underestimate effect sizes under the log-normal distribution, which is consistent with the low power of the method for this distribution (see @fig-power-main-1). In contrast, ART's estimates are spread across both sides. INT demonstrates the highest precision, followed by RNK. Both methods, however, fail to produce accurate estimates under the ordinal scale, tending to underestimate effects. Still, ART performs worse than the other methods.

@fig-effect-main-scatterplots-2 shows the same relationship but for a larger range of effects in the range $[-8, 8]$. Notice that effect sizes are severely distorted by all four methods. We invite the reader to zoom in on the lower range of values (partial $\eta^2 < 0.6$), where ART's estimates for the two non-normal distributions appear as quasi-random. Interestingly, the performance of RNK and INT also deteriorates, as the methods seem to struggle with detecting main effects in the presence of strong interactions.
...
#df <- reshapeByDesign(df, dnames, effectvars = c("effectX1","effectX2","effectX1X2"))
plotlyErrorByDesign2(df, xlab = "range of effects (g)", var = "cohensfX1", xvar = "effectX1", min = -4, max = 103, ytitle = 'R squared')
```
